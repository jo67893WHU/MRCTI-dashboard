...
function renderTrends() {
  ...
  $('#trendBody').innerHTML = pageRows.map((d,i) => { const delta=d[`${prefix}Delta`]; return `<tr data-id="${d.id}"><td>${start+i+1}</td><td><span class="record-name">${d.name || 'Name unavailable'}</span><span class="record-id">${d.id}</span></td><td>${d.st||'—'}</td><td>${d.own||'—'}</td><td class="num">${finite(d.pop)?fmt.format(Math.round(d.pop)):'—'}</td><td class="num">${fmt.format(d[`${prefix}10`])}</td><td class="num">${pct.format(d[`${prefix}Prev`])}</td><td class="num">${pct.format(d[`${prefix}Recent`])}</td><td class="num residual-${delta>=0?'positive':'negative'}">${delta>0?'+':''}${pct.format(delta)}</td><td>${delta<0?'Improved':delta>0?'Worsened':'No change'}</td></tr>`; }).join('');
